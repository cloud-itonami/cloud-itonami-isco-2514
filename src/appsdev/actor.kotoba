(ns appsdev.actor
  "ApplicationsProgrammingActor — the ISCO-08 2514 community appsdev actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one appsdev operation request
  ...
  The unconditional invariant: the ApplicationsProgrammingAdvisor can never
  directly commit a record the ApplicationsProgrammingGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [appsdev.advisor :as advisor]
            [appsdev.governor :as governor]
            [appsdev.store :as store]))

(defn build-graph
  "Build a compiled ApplicationsProgrammingActor graph. `store` implements
  `appsdev.store/Store`. `advisor` implements
  `appsdev.advisor/Advisor` (defaults to `mock-advisor`).
